...
export function generateMetadata({ params }: { params: { id: string } }): Metadata {
  return buildLotteryMetadata("en", params.id);
}
...
export default function LotteryPage({ params }: { params: { id: string } }) {
  ...
  const jsonLd = buildLotteryJsonLd("en", config.id, draws, hot);
  const breadcrumbJsonLd = buildLotteryBreadcrumbJsonLd("en", config.id);
