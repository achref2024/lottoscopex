...
export function generateStaticParams() {
  return LOTTERIES.flatMap((lottery) => {
    const draws = getDraws(lottery.id);
...
function loadDraw(id: string, date: string) {
  ...
  const prevDraw = draws[index + 1] ?? null; // chronologically before (array is newest-first)
  ...
  return { config, draw, prevDraw, nextDraw, insights, drawNumber, totalDraws: draws.length };
}
